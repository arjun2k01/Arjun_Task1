...
export default function MeterErrorPage() {
  ...
  // We initialize with empty arrays if state is missing to allow hooks to run safely
  ...
  // 1. Identify which rows have errors
  const errorRowIndices = useMemo(() => {
    const indices = new Set<number>();
    errors.forEach((e) => {
      // Row 2 in Excel = Index 0 in data
  ...
  // --- EARLY RETURN MOVED HERE ---
  // Now that all hooks have been called, we can safely redirect if data is missing.
  if (!initialState || !initialState.data) {
    return <Navigate to="/meter/upload" replace />;
  }
